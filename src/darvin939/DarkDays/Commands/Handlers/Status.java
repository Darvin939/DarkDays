	...
	public boolean perform(Player p, String[] args) throws InvalidUsage {
		if (hasPermissions(p, "status", true))
			if (PlayerInfo.isPlaying(p)) {
				PlayerData pb = (PlayerData) PlayerInfo.getPlayers().get(p.getUniqueId());
				Util.msg(p, "&b================ &2Your Progress &b==================", '/');
				Util.msg(p, "Current session:", '/');
				Util.msg(p, "  &7Players you bandaged:&6 " + pb.getPlayerHeals(), '/');
				Util.msg(p, "  &7Killed players:&6 " + pb.getPlayerKills(), '/');
				Util.msg(p, "  &7Killed zombies:&6 " + pb.getZombieKills(), '/');
				Util.msg(p, "Total:", '/');
				Util.msg(p, "  &7Players you bandaged:&6 " + pb.getPlayerHeals(), '/');
				Util.msg(p, "  &7Killed players:&6 " + pb.getPlayerKills(), '/');
				Util.msg(p, "  &7Killed zombies:&6 " + pb.getZombieKills(), '/');
